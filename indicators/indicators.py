...
# 구글 스타일, 최소한의 한글 주석
# 소수점 셋째 자리에서 반올림하여 둘째 자리까지만 보여주도록 한다.
# (즉, 결과값 = round(value, 2))
...
def calc_all_indicators(df: pd.DataFrame, cfg: dict = None) -> pd.DataFrame:
    ...
    MA, RSI, OBV, Filter, S/R, Channel_Breakout 등을 계산 후,
    각 결과를 소수점 둘째 자리로 round하여 df에 추가.
    """
    if cfg is None:
    ...
    if "MA" in cfg:
        ...
        for sp in sp_list:
            ...
            df[f"ma_{sp}"] = s.round(2)
        for lp in lp_list:
            s = calc_sma_series(df["close"], lp)
            df[f"ma_{lp}"] = s.round(2)
    ...
    if "OBV" in cfg:
        ...
        # obv 컬럼 = obv_raw의 소수점 둘째 자리 반올림
        df["obv"] = df["obv_raw"].round(2)
