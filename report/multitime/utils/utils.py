...
def get_instructions_text():
    ...
    return (
        f"현재 시간: {timestamp}\n"
        "1. 최신 데이터를 분석 후, 시장 추세를 분석하라.\n"
        "2. 현재 추세를 점수로 제시하라. 추세 점수: -100점 ~ 100점(-100점에 가까울수록 내림추세, 100점에 가까울수록 오름추세, 0점에 가까울수록 횡보)\n"
        "3. 추세 점수에 따라 추세장, 횡보장에 맞는 전략을 세우고 포지션(LONG, SHORT), 진입가, 목표가, 손절가를 제시하라.\n"
    )
