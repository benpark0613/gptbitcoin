...
# 시그널 결합 방식 설정
# "sum" (기존 합산 방식),
# "and" (모든 지표 신호가 +1이어야 매수, -1이어야 매도),
SIGNAL_COMBINE_METHOD = "sum"
# SIGNAL_COMBINE_METHOD = "and"
...
INDICATOR_CONFIG = {
    "MA": {
        ...
        # 매매시그널: short_ma > long_ma → +1, short_ma < long_ma → -1, 그 외 → 0
    },

    "RSI": {
        "lookback_periods": [7, 14, 21, 28],
        "thresholds": [[30, 70], [20, 80], [25, 75], [40, 60]],
        # 매매시그널: RSI > upper_threshold → +1, RSI < lower_threshold → -1, 그 외 → 0
    },

    "OBV": {
        "short_ma_periods": [5, 10, 20, 30, 50, 60],
        "long_ma_periods": [100, 150, 200, 250, 300],
        # 매매시그널: OBV의 단기이평 > 장기이평 → +1, 단기이평 < 장기이평 → -1, 그 외 → 0
    },
    ...
    "MACD": {
        ...
        "slow_periods": [26, 30],
        "signal_periods": [9, 12],
        # 매매시그널: MACD 라인 > 시그널 라인 → +1, MACD 라인 < 시그널 라인 → -1, 그 외 → 0
    },

    "DMI_ADX": {
        "lookback_periods": [7, 14, 20, 28],
        "adx_thresholds": [20, 25, 30, 35, 40],
        # 매매시그널: ADX > threshold일 때 +DI > -DI → +1, +DI < -DI → -1, 그 외 → 0
    },

    "BOLL": {
        "lookback_periods": [14, 20, 30, 50],
        "stddev_multipliers": [2, 2.5, 3],
        # 매매시그널: 가격 > upper_band → +1, 가격 < lower_band → -1, 그 외 → 0
    },
    ...
    "PSAR": {
        ...
        # 매매시그널: 가격이 PSAR 위 → +1, 가격이 PSAR 아래 → -1
    },

    "SUPERTREND": {
        "atr_period": [10, 14, 20],
        "multiplier": [2, 3, 4],
        # 매매시그널: 가격이 Supertrend 위 → +1, 아래 → -1
    },

    "DONCHIAN_CHANNEL": {
        "lookback_periods": [20, 30, 55, 100],
        # 매매시그널: 가격 > upper_channel → +1, 가격 < lower_channel → -1, 그 외 → 0
    },
    ...
    "STOCH": {
        ...
        # 매매시그널: K & D > upper_threshold → +1, K & D < lower_threshold → -1, 그 외 → 0
    },
    ...
    "STOCH_RSI": {
        ...
        "d_period": [3, 5],
        "thresholds": [[20, 80], [25, 75], [30, 70]],
        # 매매시그널: StochRSI > upper_threshold → +1, StochRSI < lower_threshold → -1, 그 외 → 0
    },

    "VWAP": {
        # 매매시그널: 가격 > VWAP → +1, 가격 < VWAP → -1
    },
