...
def calculate_adx(df, period=14):
    """
    pandas‑ta 라이브러리를 사용하여 ADX 보조지표를 계산합니다.
    df의 'high', 'low', 'close' 컬럼을 numeric으로 변환한 후,
    ta.adx() 함수를 호출하여 ADX 값을 계산하고, 소수점 둘째 자리까지 반올림하여 'ADX' 컬럼으로 추가합니다.
    """
    ...
    # pandas‑ta는 결과 컬럼명을 "ADX_<length>" 형태로 반환합니다.
    adx_col = f"ADX_{period}"
    df["ADX"] = adx_df[adx_col].round(2)
    return df


def add_adx_to_ohlcv(df_main, client, symbol, interval, start_date, adx_period=14):
    """
    메인 OHLCV 데이터(df_main)에 대해서만 pandas‑ta를 사용해 ADX 보조지표를 계산하고,
    결과 DataFrame을 반환합니다.
    초반 캔들에 보조지표를 강제로 채워 넣기 위해 추가 캔들 데이터를 가져오지 않습니다.
    """
    df_main = calculate_adx(df_main, period=adx_period)
    return df_main


def run(symbol, interval, start_date, end_date, adx_period=14):
