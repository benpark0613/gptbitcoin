...
def add_trend_indicators(df):
    """
    BTCUSDT 5분봉 기준, 횡보장 대응을 고려한 보조지표 세팅 예시
    (일목균형표 제거)
    """
    ...
    # 2. AMAT: fast=10, slow=20
    amat_df = ta.amat(close=df["Close"], fast=10, slow=20)
    ...
    # 10. Long Run: fast=15, slow=40
    long_run_df = ta.long_run(close=df["Close"], fast=15, slow=40)
    df = pd.concat([df, long_run_df], axis=1)

    # 11. PSAR: af=0.02, max_af=0.2
    ...
    # 13. Short Run: fast=3, slow=9
    short_run_df = ta.short_run(close=df["Close"], fast=3, slow=9)
    ...
    # 15. VHF: length=14
    vhf_df = ta.vhf(close=df["Close"], length=14)
    df = pd.concat([df, vhf_df], axis=1)

    # 16. Vortex: length=9
    vortex_df = ta.vortex(high=df["High"], low=df["Low"], close=df["Close"], length=9)
    df = pd.concat([df, vortex_df], axis=1)

    # 17. ER (Efficiency Ratio): length=10
    er_df = ta.er(df["Close"], length=10)
    df = pd.concat([df, er_df], axis=1)

    # 18. Inertia: length=10, mamode='ema'
    inertia_df = ta.inertia(df["Close"], length=10, mamode='ema')
    df = pd.concat([df, inertia_df], axis=1)

    # 19. STC: fast=12, slow=26
    stc_df = ta.stc(high=df["High"], low=df["Low"], close=df["Close"], fast=12, slow=26)
    df = pd.concat([df, stc_df], axis=1)

    # 20. MACD: fast=8, slow=21, signal=5
    macd_df = ta.macd(df["Close"], fast=8, slow=21, signal=5)
    df = pd.concat([df, macd_df], axis=1)

    # (일목균형표 Ichimoku 부분 전부 제거했습니다)
    ...
    # Stoch RSI: length=9, rsi_length=9, k=3, d=3
    ...
    # MFI: length=9
    mfi_df = ta.mfi(high=df["High"], low=df["Low"], close=df["Close"], volume=df["Volume"], length=9)
