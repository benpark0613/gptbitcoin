...
def add_trend_indicators(df):
    ...
    # ADX (1시간봉에서 좀 더 긴 추세 파악)
    df.ta.adx(
        high="High", low="Low", close="Close",
        length=20,            # 15분봉 대비 기간 늘림
        append=True
    )
    ...
    df.ta.chop(
        high="High", low="Low", close="Close",
        length=22,           # 1시간봉에서 비교적 긴 구간으로 노이즈 감소
        append=True
    )
    ...
    df.ta.kst(
        close="Close",
        roc1=9, roc2=13, roc3=15, roc4=20,
...
def main():
    ...
    df_last_10 = df_with_opentime.tail(50)
